...
var ChatMistralAI = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	...
	*/
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields);
		this._addVersion("@langchain/mistralai", "1.2.0");
	...
	*/
	...
	async *_streamChatModelEvents(messages, options, _runManager) {
		...
		async function* extractData(source, signal) {
			for await (const { data } of source) {
				if (signal?.aborted) return;
				yield data;
			}
		}
		yield* require_stream_events.convertMistralStream(extractData(streamIterable, options.signal), { streamUsage: shouldStreamUsage });
	}
