...
const redaction_js_1 = require("../utils/redaction.cjs");
...
function redactMcpServers(servers) {
    if (!Array.isArray(servers))
        return servers;
    return servers.map((server) => (0, redaction_js_1.redactOutside)(server, MCP_SERVER_SAFE_KEYS));
}
...
const wrapAnthropic = (anthropic, options) => {
    ...
    function redactAnthropicParams(params) {
        ...
        for (const key of TRANSPORT_SECRET_KEYS) {
            if (processed[key] != null) {
                processed[key] = (0, redaction_js_1.mask)(processed[key]);
            }
        }
        if (!params.system) {
            return processed;
    ...
    function processTracedInputs(inputs) {
        return (0, redaction_js_1.overParams)(inputs, redactAnthropicParams);
    }
