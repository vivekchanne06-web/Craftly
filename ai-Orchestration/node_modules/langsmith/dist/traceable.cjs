...
inputs, processInputs) => {
    ...
    const dropped = (e) => {
        if ((0, env_js_1.allowUnprocessedPayloads)()) {
            console.warn("Error occurred during processInputs. Tracing unprocessed inputs " +
                "because LANGSMITH_ALLOW_UNPROCESSED_PAYLOADS is set:", e);
            return inputs;
        }
        console.warn("Error occurred during processInputs. Dropping inputs:", e);
        return {
            [constants_js_1._PROCESSING_FAILED_KEY]: (0, constants_js_1._processingFailed)("inputs", "processInputs"),
        };
    };
    try {
        const processed = processInputs(inputs);
        return (0, asserts_js_1.isThenable)(processed)
...
async function handleRunOutputs(params) {
    ...
    const droppedOutputs = (e) => {
        if ((0, env_js_1.allowUnprocessedPayloads)()) {
            console.error("Error occurred during processOutputs. Tracing unprocessed outputs " +
                "because LANGSMITH_ALLOW_UNPROCESSED_PAYLOADS is set:", e);
            return unprocessed;
        }
        console.error("Error occurred during processOutputs. Dropping outputs:", e);
        return {
            [constants_js_1._PROCESSING_FAILED_KEY]: (0, constants_js_1._processingFailed)("outputs", "processOutputs"),
        };
    };
    const endOutputsDropped = async (e) => {
...
const getTracingRunTree = (runTree, inputs, getInvocationParams, processInputs, extractAttachments) => {
    ...
    if ((0, asserts_js_1.isThenable)(processedInputs)) {
