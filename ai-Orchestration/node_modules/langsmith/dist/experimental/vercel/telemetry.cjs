...
function droppedPayload(payload, hook, raw, e) {
    if ((0, env_js_1.allowUnprocessedPayloads)()) {
        console.error(`Error in ${hook}. Tracing unprocessed ${payload} because ` +
            "LANGSMITH_ALLOW_UNPROCESSED_PAYLOADS is set:", e);
        return raw;
    }
    console.error(`Error in ${hook}. Dropping ${payload}:`, e);
    return { [constants_js_1._PROCESSING_FAILED_KEY]: (0, constants_js_1._processingFailed)(payload, hook) };
}
...
function LangSmithTelemetry(config) {
    ...
    const onStart = async (event) => {
        if (!(0, env_js_2.isEnvTracingEnabled)(tracingEnabled))
            return;
        ...
        if (event.recordInputs !== false) {
            ...
            if (processChildLLMRunInputs) {
                try {
                    ...
                    inputs = droppedPayload("inputs", "processChildLLMRunInputs", inputs, e);
                }
            }
        }
        ...
        if (event.recordOutputs !== false) {
            ...
            if (processChildLLMRunOutputs) {
                try {
                    ...
                    outputs = droppedPayload("outputs", "processChildLLMRunOutputs", outputs, e);
                }
            }
        }
        if (harnessState != null && Array.isArray(event.content)) {
